...
UPSTREAM_REGEX2 = re.compile(r"\[\s*Upstream\s*commit\s*([0-9a-z]{7,40})\s*\]",flags=re.IGNORECASE)
repo_path = "../linux"
repo = Repo(repo_path)
...
def check_upstream(message):
    """Extracts all commit IDs from the 'Fixes:' pattern in the message."""
    match1 = UPSTREAM_REGEX.findall(message if message else "")
    match2 = UPSTREAM_REGEX2.findall(message if message else "")
    match1.extend(match2)
    
    return match1[0] if len(match1) > 0 else None
...
def main(input_json, output_json):
    ...
    for partial_commit_id in commit_ids:
        ...
        if result:
            ...
            print(upstream_id)
            if upstream_id:
                upstream_id, commit_message = get_full_commit_info(upstream_id)
            results.append(
                {
                    "VFC": upstream_id if upstream_id else full_commit_id, 
                    "VIC": fixes_ids if fixes_ids else [],
                    "origina": full_commit_id if upstream_id else ""
                }
                )
...
input_json = "all_filtered.json"  # Input JSON file containing partial commit IDs
output_json = "output2.json"  # Output JSON file to save the results
